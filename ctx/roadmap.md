...
### 완료 조건 (AC)
1. Android + iOS 기기에서 터치로 조작 가능
2. UI가 모바일 화면에 맞게 표시됨 (Safe Area 고려)
...
### 완료 조건 (AC)
1. 게임이 10분간 에러 없이 실행됨 (모든 플랫폼)
2. **Windows + Android + iOS** 빌드 성공
3. README에 플랫폼별 빌드/실행 방법 문서화
...
- **M3** (Day 7): 크로스 플랫폼 빌드 완성 (Windows + Android + iOS)
