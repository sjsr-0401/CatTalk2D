...
public class StatisticsService
{
    ...
    public SessionStatistics CalculateStatistics(LogSession session)
    {
        ...
        stats.FeedCount = session.Records.Count(r => r.ParsedActionType == LogActionType.Feed);
        stats.PetCount = session.Records.Count(r => r.ParsedActionType == LogActionType.Pet);
        stats.PlayCount = session.Records.Count(r => r.ParsedActionType == LogActionType.Play);
        stats.TalkCount = session.Records.Count(r => r.ParsedActionType == LogActionType.Talk);
        stats.MonologueCount = session.Records.Count(r => r.ParsedActionType == LogActionType.Monologue);
    ...
    public List<(DateTime time, float value)> GetTimeSeriesData(
        LogSession session,
        Func<LogCatStateSnapshot, float> selector)
    {
    ...
    /// 행동별 카운트
    /// </summary>
    public Dictionary<LogActionType, int> GetActionCounts(LogSession session)
    {
        return session.Records
    ...
    public Dictionary<string, float> GetLatestPersonality(LogSession session)
    {
        ...
        {
            { "장난기", latestState.Playful },
            { "소심함", latestState.Shy },
            { "까칠함", latestState.Aggressive },
            { "호기심", latestState.Curious }
        };
